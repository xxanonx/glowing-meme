...
void loop() {
  ...
  //Serial.print("\n");
  delay(500);
  calc_Q2 = int(sortLast[int(sizeOfLast * 0.5)]);
  calc_Q1 = int(sortLast[int(sizeOfLast * 0.25)]);
  calc_Q3 = int(sortLast[int(sizeOfLast * 0.75)]);
  IQR = calc_Q3 - calc_Q1;
  LowerFence = int(calc_Q1 - (1.5 * IQR));
  UpperFence = int(calc_Q3 + (1.5 * IQR));
